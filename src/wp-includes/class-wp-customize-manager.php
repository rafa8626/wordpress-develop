...
final class WP_Customize_Manager {
	...
	public function post_value( $setting, $default = null ) {
		...
		// @todo Continue to recognize $_POST['customized'].
	...
	public function customize_preview_init() {
		...
		/*
		 * Start output buffering both to improve retention of scroll position and
		 * also to allow
		 */
		...
		// Use our own wrapper for wp_ob_end_flush_all.
		remove_action( 'shutdown', 'wp_ob_end_flush_all', 1 );
		add_action( 'shutdown', array( $this, 'preview_shutdown' ), 1 );
	...
	public function preview_shutdown() {
		...
		if ( ! empty( $last_error ) && in_array( $last_error['type'], array( E_ERROR, E_USER_ERROR, E_RECOVERABLE_ERROR ), true ) ) {
	...
	public function update_transaction() {
		...
		// Send back sanitized settings so that the UI can be updated to reflect the PHP-sanitized values.
		$response = array(
			'transaction_uuid' => $this->transaction->uuid,
			'transaction_settings' => $this->transaction->data(),
		);
